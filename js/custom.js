...
function getdata() {
    ...
    $.ajax({
        ...
        success: function(vals) {
            var finaldata = [];
            var arraydata = JSON.parse(vals);
            var sortdata = arraydata.sort(function(a, b) {
                return (new Date(b.datetime)) - (new Date(a.datetime))
            });
            var str = JSON.stringify(sortdata);
            _.each(sortdata, function(value, key) {
                _.each(value, function(value, key) {
                    if (key == 'name') {
                        finaldata.push("<div class='col-md-12 wishes'><h4>" + value + "</h4></div>");
                    }
                    if (key == 'message') {
                        finaldata.push("<div class='col-md-12 wishes'><p>" + value + "</p></div>");
                    }
                    if (key == 'datetime') {
                        finaldata.push("<div class='col-md-12 wishes'><h6>" + value + "</h6></div>");
                    }
...
setInterval(function() {
    var timespan = countdown(new Date("02/04/2018"), new Date());
